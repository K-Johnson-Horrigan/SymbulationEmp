...
EMP_BUILD_CONFIG(SymConfigBase,
    ...
    VALUE(PHYLOGENY_TAXON_TYPE, size_t, 0, "What are phylogeny taxa based on? 0 = binned genotypes values, 1 = exact phenotype values, 2 = bitset tag (for tag matching condition)"),
    ...
    VALUE(TAG_DISTANCE_EVOLVES, bool, 0, "Should each host have a tag permissiveness that evolves as hosts reproduce? If yes, starting tag permissiveness values will be set to the TAG_DISTANCE config option. (0 for no, 1 for yes)"),
    VALUE(TAG_DISTANCE_MUTATION_SIZE, double, 0.005, "Standard deviation of the distribution to mutate by for host tag permissiveness values"),
