...
public:
  ...
  ~Symbiont() {
    if(my_config->PHYLOGENY() == 1) {
      my_world->GetSymSys()->RemoveOrg(my_taxon.Cast<emp::Taxon<taxon_t::info_t, datastruct::SymbiontTaxonData>>());
      if (my_config->STORE_EXTINCT() && my_taxon->GetOriginationTime() == my_taxon->GetDestructionTime() && my_taxon->GetTotalOffspring() == 0) {
        my_world->GetSymSys()->outside_taxa.erase(my_taxon.Cast<emp::Taxon<taxon_t::info_t, datastruct::SymbiontTaxonData>>());
        my_taxon.Delete();
      }
    }
  }
 ...
 /**
   ...
   */
   emp::Ptr<emp::Taxon<taxon_t::info_t, datastruct::TaxonDataBase>> GetTaxon() {return my_taxon;}
    ...
    */
   void SetTaxon(emp::Ptr<emp::Taxon<taxon_t::info_t, datastruct::TaxonDataBase>> _in) {my_taxon = _in;}
